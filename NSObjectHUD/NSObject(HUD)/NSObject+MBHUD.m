 ...
 */
+ (MBProgressHUD *)MB_showMessage:(NSString *)message{
    return [self MB_showMessage:message toView:nil];
}
/*!
...
- (void)MB_alert:(NSString *)text{
    ...
    [[self class] MB_showText:text icon:nil toView:nil];
}
...
- (void)MB_showErrorAlert:(NSString *)text{
    [[self class] MB_showError:text];
}

- (void)MB_showSuccessAlert:(NSString *)text{
    [[self class] MB_showSuccess:text];
}
